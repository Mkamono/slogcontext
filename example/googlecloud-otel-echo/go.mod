module googlecloud-otel-echo-example

go 1.25.0

replace (
	github.com/Mkamono/slogcontext/slogcontext => ../../slogcontext
	github.com/Mkamono/slogcontext/slogcontext/otel => ../../slogcontext/otel
	github.com/Mkamono/slogcontext/slogcontext/wrapper/echo => ../../slogcontext/wrapper/echo
)

require (
	github.com/Mkamono/slogcontext/slogcontext v0.0.0
	github.com/Mkamono/slogcontext/slogcontext/otel v0.0.0-00010101000000-000000000000
	github.com/Mkamono/slogcontext/slogcontext/wrapper/echo v0.0.0-00010101000000-000000000000
	github.com/labstack/echo/v4 v4.15.1
	go.opentelemetry.io/otel v1.43.0
	go.opentelemetry.io/otel/sdk v1.43.0
)

require (
	github.com/cespare/xxhash/v2 v2.3.0 // indirect
	github.com/go-logr/logr v1.4.3 // indirect
	github.com/go-logr/stdr v1.2.2 // indirect
	github.com/google/uuid v1.6.0 // indirect
	github.com/labstack/gommon v0.4.2 // indirect
	github.com/mattn/go-colorable v0.1.14 // indirect
	github.com/mattn/go-isatty v0.0.20 // indirect
	github.com/valyala/bytebufferpool v1.0.0 // indirect
	github.com/valyala/fasttemplate v1.2.2 // indirect
	go.opentelemetry.io/auto/sdk v1.2.1 // indirect
	go.opentelemetry.io/otel/metric v1.43.0 // indirect
	go.opentelemetry.io/otel/trace v1.43.0 // indirect
	golang.org/x/crypto v0.46.0 // indirect
	golang.org/x/net v0.48.0 // indirect
	golang.org/x/sys v0.42.0 // indirect
	golang.org/x/text v0.32.0 // indirect
)
